...
export default function FeatureAccordion() {
  ...
  return (
    <>
      ...
      <div className="hidden h-135 w-full gap-3 md:flex">
        {features.map((feature, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="absolute left-7 top-7 font-display text-4xl text-white/90">
                {feature.number}.
              </span>

              {/* Collapsed label — hidden once the panel opens */}
              <span
                className={`absolute bottom-6 left-7 right-3 font-display text-xl leading-tight text-white transition-opacity duration-300 ${
              ...
              <div
                className={`absolute inset-x-9 bottom-9 transition-all duration-500 ease-out ${
                  isOpen
      ...
      {/*
        ...
        The expanding-panel metaphor needs horizontal room. On a phone the four
        panels would share one column and clip their own body copy, so each
        feature becomes a plain image-over-text card instead.
      */}
      <div className="flex flex-col gap-12 md:hidden">
        {features.map((feature) => (
          <article key={feature.number}>
            <div className="relative aspect-16/10 w-full overflow-hidden rounded-3xl bg-fog">
              <img
                src={feature.image}
                alt={feature.alt}
              ...
              <span className="absolute left-5 top-5 font-display text-3xl text-white/90">
                {feature.number}.
              </span>
            </div>

            <h3 className="mt-5 font-display text-[26px] font-normal leading-tight tracking-tight text-[#18181B]">
              {feature.title}
            </h3>
